...
class CompleteFallback:
        ...
        for attempt in range(retries):
            try:
                ...
                if attempt < retries - 1:
                    print(f"⏰ 等待 {delay/1000} 秒後重試... (指數退避)")
    ...
    def _call_single_model(self, model: str, messages: List[Dict[str, Any]]) -> Dict[str, Any]:
        ...
        try:
            with urllib.request.urlopen(req, timeout=self.timeout/1000) as response:
...
# Ricky 建議嘅實用函數
async def with_retry(fn, retries: int = 3):
    """
    Ricky 建議嘅Retry + Backoff函數
    """
    delay = 1000
...
def call_llm_fallback(messages: List[Dict[str, Any]], api_key: str = None) -> Dict[str, Any]:
    ...
    便捷嘅LLM調用函數
    """
    fallback = CompleteFallback(api_key)
    return fallback.call_llm_with_retry(messages)

# 測試函數
def test_complete_fallback(api_key: str = None):
    """
    測試完整嘅fallback系統
    """
    if not api_key:
...
if __name__ == "__main__":
    ...
    result = test_complete_fallback(api_key)
