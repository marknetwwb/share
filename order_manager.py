...
class ACSOrderManager:
    def __init__(self):
        ...
        self.price_columns = [
            'Location', '九龙', '港岛', '機場/東涌', '大埔/上水', 
            '西貢', '元朗市/落馬洲', '將軍澳', '沙田', '郵碼'
        ]
...
def main():
    ...
    price_file = 'ACS_Driver_Price_Table.xlsx'
    ...
    if choice == '1':
        ...
        time = input("時間 (HH:MM 或 HH:MM-HH:MM): ")
        route = input("路線 (例: 中環 → 尖沙咀): ")
        passenger = input("乘客姓名: ")
        cars_required = int(input("需要車輛數量: "))
        customer_info = input("客戶資料: ")
        overtime_fee = int(input("超時費: "))
        parking_fee = int(input("停車費: "))
        
        order_data, driver_details = manager.create_order(
            date, time, route, passenger, cars_required, 
            customer_info, overtime_fee, parking_fee
        )
        
        # 添加到文件
        updated_orders = manager.add_order_to_file(order_file, order_data)
        ...
        for driver in driver_details:
            print(f"姓名: {driver['name']}, 車牌: {driver['plate']}, 手機: {driver['mobile']}")
        
        # 顯示更新後的訂單列表
        manager.display_orders(updated_orders)
